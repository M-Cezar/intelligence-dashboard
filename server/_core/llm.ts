...
const resolveApiUrl = () => {
  if (!ENV.forgeApiUrl || ENV.forgeApiUrl.trim().length === 0) throw new Error("LLM API URL is not configured");
  return `${ENV.forgeApiUrl.replace(/\/$/, "")}/v1/chat/completions`;
};
const assertApiKey = () => { if (!ENV.forgeApiKey) throw new Error("LLM API key is not configured"); };
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  const response = await fetch(resolveApiUrl(), { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${ENV.forgeApiKey}` }, body: JSON.stringify(payload) });
  if (!response.ok) {
